 ...
 *
 * Значительная доля затрат на реализацию интерфейса Serializable связана с тем, что это уменьшается возможность
 * изменения реализации класса в последующих версиях.
 *
 * Второе неудобство от реализации интерфейса Serializable заключается в том , что повышается вероятность появления
 * ошибок и дыр в защите.
 *
 * Третим неудобством реализации интерфейса Serializable связано с тем , что выпуск новой версии класса тянет за собой большую
 * работу по тестированию.
 *
 * Классы, предназначенные для наследования (статья 17) , редко должны реализовывать Serializable , а интерфейсы - редко
 * его расширять.
 *
 * Для несериализуемого класса , который предназначен для наследования, вы должны рассмотреть
 * возможность создания конструктора без параметров.
 *
 */
...
class Foo extends AbstractFoo implements Serializable
{
    ...
    //Конструктор не используется никаких причудливых механизмов
