 ...
 *
 * Зная о трудности использования wait и notify, вам вместо этого следует испльзовать высокоурованевые утилиты параеллельности.
 * - Структура экзекуторов
 * - Параллельные колекции
 * - Синхронизаторы
 *
 * Невозможно исключить паралельнную деятельность из параельной коллекции , ее блокировка будет иметь дествие ,
 * а только затормозит выполнение программы.
 *
 * BlockingQueue - блокирующие очереди для рабочих очередей
 *
 * Синхронизаторы - объекты которые дают ждать потокам друг друга , позволяя им кординировать их деятельность .
 * CountDownLatch or Semaphore
 *
 * wait всегда запускается в цыкли while !!!
 */
public class Item69
{
...
//        });
    ...
    //Паралельная традиционая схема поверх ConcurrentMap - не оптимальная
    public static String intern(String s)
    {
        return map.putIfAbsent(s,s);
    }

    //Паралельная традиционая схема поверх ConcurrentMap - работает быстрее
