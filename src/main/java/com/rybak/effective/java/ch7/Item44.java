 ...
 *
 * Чтобы должным образом документировать API, следует предварять doc-коментарием каждую представляемую пользователем
 * делкларацию класса, интерфейса, конструктора, метода или поля.
 *
 * Doc-комментарий для метода должен локанично описывать соглания между этими методами и его клиентами.
 * Что делает данный метод? НЕ КАК
 * Прекондишен @throws Посткондишен
 *
 * Безопасность класса при работе с потоками
 *
 * @param   - представляют собой именную констркуцию
 * @return  - по анлийски noun phrase
 *
 * @throw должен состоять из слов if
 *
 * Когда вы пишете документацию по обобщеным типам или методам , обязательно документируйте все параметры типа:
 *
...
public class Item44<E>
{
    ...
    /**
     *
     * методы глагольный конструкции создает , возвращает
     *
     * классы интерфейсы поля должны именной конструкции
     *
     * [summary description]Возвращает элемент, который занимает заданую позицию в данном списке.
